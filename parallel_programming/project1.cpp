...
int main( int argc, char *argv[] )
{
	...
	#pragma omp parallel for reduction(+:sum),private(iu,iv,height)
	for( int i = 0; i < NUMNODES*NUMNODES; i++ )
	{
		...
		if ((iv < (NUMNODES - 1) && iv > 0) && (iu < (NUMNODES - 1) && iu > 0)) {
			sum += height * fullTileArea;
		} else {
			if ((iv == 0 || iv == (NUMNODES - 1)) && (iu == 0 || iv == (NUMNODES - 1))) {
				sum += height * (float)(fullTileArea / 2);
			} else {
				sum += height * (float)(fullTileArea / 4);
			}
		}
	}

	double end = omp_get_wtime();
	double mheight = (double)NUMNODES*NUMNODES/(end-start)/1000000.;

	printf("# of Threads: %i\n", NUMT);
	printf("# of Nodes: %i\n", NUMNODES);
	printf("Volume: %f\n", sum);
	printf("Time: %10.2lf msecs\n", 1000000. * (end - start));
	printf("Efficiency: %10.2lf MegaHeights\n\n", mheight);
}
